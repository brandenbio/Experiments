...
function callServer(answer)                                   // Calls server to get stimulus objects
{
	...
	else
	{
        ...
        $.getJSON(url, function(data) {
			...
			else { setTask(data); displayProblem(data); }           // displayProblem is defined in showStimuli.html
        });
    }
}
...
function resetTask(task) {
    switch (task) {
        ...
        case "sentencecompletion-task":
            ...
            break;
        case "drag-fillintheblank-task":
            clearTrial();
        case "click-fillintheblank-task":
            clearTrial();
            break;
    }
}
...
function parseResponse(raw) {
    //var temp1 = $(raw).html();
    //var temp2 = temp1.match(/>.*</g).toString().replace(/</g, '').replace(/>/g, '');
    //var answer = temp2.replace(/,/g, ' ');
    var answer = $(raw).text();
    return answer;
}

function respondFillInTheBlank() {
    endTime = $.now();
    //callServer(parseResponse($("#flexChild2")));
    callServer(parseResponse($("#blank")));
}

function setFillInTheBlank(option, value) {
    $(option).html(value);
    $(option).val(value);
}
